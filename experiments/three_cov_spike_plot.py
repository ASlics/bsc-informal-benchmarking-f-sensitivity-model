...
DATA_PATH = os.path.join(os.path.dirname(__file__), "data", "three_cov_spike.json")
OUT_PATH = os.path.join(os.path.dirname(__file__), "three_cov_spike_experiment.png")
...
def main():
    if not os.path.exists(DATA_PATH):
        print(f"No data at {DATA_PATH}.\n"
              f"Run the DGP first:  python {os.path.join('experiments', 'three_cov_spike_run.py')}")
        sys.exit(1)
    ...
    handles = [Line2D([0], [0], color=base_color, ls="--", lw=1.6, label="0")]
    handles += [Line2D([0], [0], color=amp_color[int(i)], lw=1.4, label=f"{amps[i]:.1f}") for i in nz]
    leg = axD.legend(handles=handles, loc="upper right", ncol=3, handlelength=1.1,
                     columnspacing=0.8, labelspacing=0.25, handletextpad=0.4, framealpha=0.9,
                     title=r"$X_0$ spike height", title_fontsize=5.5)
    ...
    axG.set_xticks(pos); axG.set_xticklabels(xlabels)
    axG.set_xlabel(r"$X_0$ spike height")
    ...
    for p in pos:
        axR.text(p, rho_bench[p] + rho_bench_sd[p] + 0.0015, f"{rho_bench[p]:.3f}",
                 ha="center", va="bottom", fontsize=5.5)
    axR.set_xticks(pos); axR.set_xticklabels(xlabels)
    axR.set_xlabel(r"$X_0$ spike height")
