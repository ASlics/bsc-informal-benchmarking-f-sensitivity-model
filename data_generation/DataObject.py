...
class DataObject:
    ...
    def __init__(self, data: pd.DataFrame):
        ...
        treated_data_frequency = data[data["T"] == 1].groupby(self.x_features, as_index=False).size().rename(columns={"size": "treated_size"})
    ...
    def probability_of_x(self, x: pd.Series) -> float:
        ...
        return float(selection["size"].iloc[0]) / sum(self.X["size"])
    ...
    def probability_of_x_given_t(self, x: pd.Series, treatment: int) -> float:
        ...
        if treatment == 0:
            ...
            if len(selection) == 0:
                return 0.0
            return float(selection["size"].iloc[0]) / sum(self.X_0["size"])
        selection = self.X_1[np.all(self.X_1[self.x_features] == x[self.x_features], axis=1)]
        if len(selection) == 0:
            return 0.0
        return float(selection["size"].iloc[0]) / sum(self.X_1["size"])
